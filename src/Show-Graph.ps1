...
Function Show-Graph {
    ...
    PROCESS {
        ...
        $NumOfDatapoints = $Points.Count
        $Metric = $Points | Measure-Object -Maximum -Minimum
        ...
        $LengthOfMaxYAxisLabel = (($Points | Measure-Object -Maximum).Maximum).tostring().length
        ...
        switch($Type){
            'Bar'       {$Array = Get-BarPlot -Datapoints $Points -Step $YAxisStep -StartOfRange $StartOfRange -EndofRange $EndofRange }
            'Scatter'   {$Array = Get-ScatterPlot -Datapoints $Points -Step $YAxisStep -StartOfRange $StartOfRange -EndofRange $EndofRange }
            'Line'      {$Array = Get-LinePlot -Datapoints $Points -Step $YAxisStep -StartOfRange $StartOfRange -EndofRange $EndofRange }
        }
